...
import ThreeServices from '@/components/three-services'
          ...
          <div className="relative max-w-7xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-20 items-center">
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
                <span className="inline-flex items-center gap-2 glass px-5 py-2.5 rounded-full mb-8 text-xs text-indigo-600 font-bold tracking-widest uppercase"><Sparkles size={13} /> Programs</span>
                <h1 className="text-5xl md:text-6xl lg:text-[4.2rem] font-bold leading-[1.05] mb-8 tracking-tight">
                  <span className="text-slate-900">Our Premium</span><br />
              ...
              {services.map((s, i) => {
                ...
                return (
                  <motion.div
                    ...
                    layout
                    className={`glass-card overflow-hidden group cursor-pointer card-3d ${isExpanded ? '!border-indigo-400/40 glow-purple' : ''}`}
                    ...
                    <div className="p-7">
                      <div className="flex items-start justify-between mb-6">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-amber-600 font-bold text-sm">{s.rating}</span>
                        </div>
                      </div>

                      <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-indigo-600 transition-colors">{s.title}</h3>
                      <p className="text-sm text-slate-600 mb-5 leading-relaxed">{s.description}</p>
                      ...
                      {isExpanded && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                          <div className="px-7 pb-7 pt-3 border-t border-slate-100">
                            <p className="text-slate-600 text-sm leading-relaxed mb-5">{s.details}</p>
                            <div className="flex flex-wrap gap-3 mb-6">
                              {s.features.map((f) => (
                                <span key={f} className="flex items-center gap-1.5 text-xs text-emerald-600 font-semibold">
          ...
          <div className="relative z-10 max-w-5xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <p className="text-lg text-slate-600 mb-12 leading-relaxed">Schedule a free 1-on-1 consultation with our career advisors. We&apos;ll help you find the perfect program.</p>
                <button className="btn-primary text-lg flex items-center gap-3 group !py-4 !px-10">
                  Book Free Consultation <ArrowRight size={18} className="group-hover:translate-x-1.5 transition-transform" />
                </button>
                <p className="mt-6 text-xs text-slate-500">✦ No commitment required · Takes 15 minutes</p>
              </motion.div>
